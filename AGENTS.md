...
`additive-migrations`, `no-silent-coercion`, `no-raw-colors`,
`no-host-literals`, alongside the existing 37. The last three check only lines
this branch added, so the pre-existing backlog stays a separate cleanup. Each
has a documented opt-out pragma, and every opt-out is a decision a reviewer
should see.
