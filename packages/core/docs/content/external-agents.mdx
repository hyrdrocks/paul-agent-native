...
The server id in that block (`mail` above) defaults to `agent-native-<first label of your hostname>`. Set `MCP_SERVER_NAME` in the deployment's environment to choose it yourself — the connect page, the copyable config, and device-flow grants all use the same value. An app that passes `mcpConnectServerName` to `createCoreRoutesPlugin` keeps that name; the environment variable only fills in when the option is absent.
