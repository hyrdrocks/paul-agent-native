...
L'id de serveur dans ce bloc (`mail` ci-dessus) vaut par défaut `agent-native-<premier label de votre nom d'hôte>`. Définissez `MCP_SERVER_NAME` dans l'environnement du déploiement pour le choisir vous-même — la page de connexion, la configuration copiable et les autorisations du flux d'appareil utilisent la même valeur. Une application qui passe `mcpConnectServerName` à `createCoreRoutesPlugin` conserve ce nom ; la variable d'environnement n'intervient que si l'option est absente.
