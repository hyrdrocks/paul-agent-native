...
## Ce que voit l'utilisateur {#user-flow}

Cross-App SSO doit donner l'impression de passer d'un produit à l'autre dans un même espace de travail, et non de créer un deuxième compte. Par exemple :

1. Connectez-vous à [Dispatch](https://dispatch.agent-native.com), ou à toute application qui utilise déjà l'identité de l'espace de travail.
2. Ouvrez [Mail](https://mail.agent-native.com), [Calendar](https://calendar.agent-native.com) ou une autre application activée dans un nouvel onglet.
3. L'application n'envoie le navigateur vers Dispatch que si aucune session Dispatch n'existe. Si la session existe déjà, la redirection est pratiquement invisible.
4. Revenez dans l'application déjà connectée, avec la même adresse e-mail, les mêmes données et le même contexte d'organisation.

La distinction importante est qu'il s'agit **d'une seule connexion, et non d'un cookie partagé**. Chaque application hébergée séparément continue de créer et de gérer sa propre session locale. Dispatch confirme l'identité de l'utilisateur à l'entrée de l'application, puis celle-ci utilise son authentification locale et ses contrôles d'accès habituels.

Pour une démonstration produit, enregistrez exactement cette séquence : connectez-vous une fois dans Dispatch, ouvrez deux autres applications hébergées, montrez qu'aucune ne demande de nouveaux identifiants, puis déconnectez-vous de l'une pour montrer que la session de l'autre reste indépendante. La limite de sécurité devient ainsi visible.
...
<Diagram id="doc-block-1w6goia" title={"Flux de fédération d'identité"} summary={"Dispatch authentifie l'humain et renvoie une assertion signée de courte durée d'une chose : l'e-mail vérifié. L'application est liée par e-mail et crée sa propre session locale."}>

```html
<div class="diagram-sso">
  <div class="diagram-card" data-rough>
    <strong>Application cliente</strong
  ...
  <div class="diagram-card" data-rough>
    <strong>Callback de l’application</strong
    ><small class="diagram-muted"
      >verify signature · scope:identity · exp &le; 2 min</small
    ><span class="diagram-pill ok">Association JIT par e-mail</span
...
1. **Application → Dispatch (autorisation).** La route `GET /_agent-native/identity/login` de l’application (celle du bouton « Se connecter avec Agent-Native ») construit la redirection et envoie l’utilisateur à l’autorité d’identité :
   ...
   `redirect_uri` est la propre route `GET /_agent-native/identity/callback` de cette application — celle qui reçoit le jeton signé à l’étape 3.

<Endpoint id="cross-app-sso-identity-authorize-fr-fr" title={"Point de terminaison d'autorisation d'identité"} method="GET" path="/_agent-native/identity/authorize" summary={"Dispatch (autorité d'identité) authentifie l'humain et le redirige avec un jeton d'identité signé"} auth={"Session de répartition (connexion interactive si aucune)"} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "L'identifiant de l'application demandeuse."
  },
  {
    "name": "redirect_uri",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "Callback de l’application URL. Validé par rapport à une liste blanche stricte (`*.agent-native.com` ou localhost par défaut)."
  },
  {
    "name": "state",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "L'état CSRF a été renvoyé lors de la redirection."
  }
]} responses={[
  {
    "status": "302",
    "description": "Redirection vers `redirect_uri` portant une identité signée `A2A_SECRET` de courte durée JWT (`scope: \"identity\"`, `exp` ≤ 2 minutes) plus l'original `state`."
  },
  {
    "status": "400",
    "description": "Échec de la validation de la liste d'autorisation `redirect_uri` (origine croisée, `//host` relative au schéma ou suffixe non répertorié)."
  }
]}>
...
4. **L’application vérifie et lie le compte juste à temps par e-mail.** La route `GET /_agent-native/identity/callback` vérifie la signature du jeton avec `A2A_SECRET`, contrôle `scope: "identity"` et `exp`, puis effectue **une liaison juste à temps strictement par e-mail vérifié** :
...
<Callout id="doc-block-yjxbff" tone="success">

**Activation sûre, restauration sécurisée.** Les écritures d'identité sont **additifs uniquement** : un compte de même adresse e-mail existant est réutilisé sans modification et un nouvel e-mail insère simplement une nouvelle ligne. Il n'y a aucun changement de schéma et rien à migrer, donc l'activation ou la désactivation de `AGENT_NATIVE_IDENTITY_HUB_URL` est entièrement réversible à tout moment, par application.

</Callout>

Le lien juste à temps est une décision unique entièrement saisie sur l’e-mail vérifié :

<Diagram id="doc-block-xxejxz" title={"Décision JIT-link"} summary={"Le lien est saisi sur l'e-mail vérifié et est uniquement additif : les comptes existants sont réutilisés sans modification, les nouveaux e-mails créent un nouvel utilisateur local."}>

```html
<div class="diagram-jit">
  <div class="diagram-node" data-rough>
    E-mail vérifié<br /><small class="diagram-muted"
      >depuis un JWT d’identité signé</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-branch">
    ...
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>Émettre une session locale normale</div>
  </div>
</div>
```
