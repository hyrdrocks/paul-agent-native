...
- **`type`** — `"content"` (par défaut) conserve le travail dans l'agent intégré, qui utilise les outils de l'application. `"code"` l'achemine vers un frame distinct capable de modifier le dépôt ; l'écriture n'est disponible que si l'hôte accorde intentionnellement des outils repository/workspace. Voir [Frames](/docs/frames).
