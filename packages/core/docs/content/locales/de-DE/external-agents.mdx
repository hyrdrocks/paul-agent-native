...
Die Server-ID in diesem Block (oben `mail`) ist standardmäßig `agent-native-<erstes Label Ihres Hostnamens>`. Setzen Sie `MCP_SERVER_NAME` in der Umgebung des Deployments, um sie selbst zu wählen – die Connect-Seite, der kopierbare Config-Block und Device-Flow-Grants verwenden denselben Wert. Eine App, die `mcpConnectServerName` an `createCoreRoutesPlugin` übergibt, behält diesen Namen; die Umgebungsvariable greift nur, wenn die Option fehlt.
