---
title: "Toolkit"
description: "Ein ausgewogener Katalog wiederverwendbarer Grundlagen, Editoren und Leinwände, Agentenkontext, Governance, Workspace-Chrome, Betrieb und Toolkit-Fähigkeitsmodule."
---

<!-- i18n-docs-ignore -->

# Toolkit

Toolkit ist das Regal wiederverwendbarer, praxiserprobter Bausteine für
hochwertige agentische Anwendungen. Es macht wiederkehrende Produktarbeit zu
Standards, ohne ein einzelnes App-Muster ins Zentrum des Katalogs zu stellen.

Dieser Abschnitt ist die zentrale Entdeckungsfläche für wiederverwendbare App-UI.
Eine Oberfläche gehört hierher, weil sie wiederverwendbar ist, auch wenn Core
oder ein Template sie derzeit besitzt. Besitzlabels und Importpfade machen diese
Grenze klar, ohne zusammenhängende UI-Hinweise über die Website zu verteilen.

Each kit packages the UI patterns, action contracts, app-state expectations,
access boundaries, and agent-visible behavior that users expect to work
consistently. Apps still own their domain model and visual identity; Toolkit
make the common product systems feel complete from the first version instead of
being rebuilt in slightly different ways every time.

<WireframeBlock id="doc-block-toolkits-index">
  <Screen
    surface="desktop"
    html={
      "<div style='min-height:560px;box-sizing:border-box;display:grid;grid-template-columns:200px 1fr 280px;gap:16px'><aside class='wf-card' style='display:flex;flex-direction:column;gap:8px;padding:14px'><div style='display:flex;align-items:center;gap:8px'><div style='width:26px;height:26px;border-radius:8px;background:var(--wf-accent-soft);display:flex;align-items:center;justify-content:center;font-size:12px;flex:none'>A</div><strong style='flex:1'>Acme</strong><span data-icon='chevronDown'></span></div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px;border-radius:8px;background:var(--wf-accent-soft)'><span data-icon='search'></span> Home</div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px'><span data-icon='edit'></span> Documents</div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px'><span data-icon='user'></span> Team</div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px'><span data-icon='settings'></span> Settings</div><div style='flex:1'></div><div style='display:flex;align-items:center;gap:8px;padding-top:10px;border-top:1.4px solid var(--wf-line)'><div style='width:24px;height:24px;border-radius:999px;background:var(--wf-accent-soft);flex:none'></div><small style='flex:1'>Steve</small><span data-icon='dots'></span></div></aside><main class='wf-card' style='display:flex;flex-direction:column;gap:14px'><div style='display:flex;align-items:center;gap:10px'><h2 style='margin:0'>Q3 Forecast</h2><div style='flex:1'></div><div style='display:flex'><div style='width:26px;height:26px;border-radius:999px;background:var(--wf-accent-soft);border:2px solid var(--wf-card);flex:none'></div><div style='width:26px;height:26px;border-radius:999px;background:var(--wf-accent-soft);border:2px solid var(--wf-card);margin-left:-8px;flex:none'></div><div style='width:26px;height:26px;border-radius:999px;background:var(--wf-accent);border:2px solid var(--wf-card);margin-left:-8px;flex:none'></div></div><button class='primary'><span data-icon='user'></span> Share</button></div><div class='wf-box' style='display:flex;flex-direction:column;gap:10px'><small class='wf-muted'>Revenue by month</small><div style='display:flex;align-items:flex-end;gap:10px;height:120px'><div style='flex:1;height:45%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:65%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:55%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:80%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:95%;background:var(--wf-accent);border-radius:4px'></div></div></div><div style='display:flex;gap:10px'><div class='wf-box' style='flex:1'><small class='wf-muted'>ARR</small><br/><strong>$4.2M</strong></div><div class='wf-box' style='flex:1'><small class='wf-muted'>Win rate</small><br/><strong>28%</strong></div><div class='wf-box' style='flex:1'><small class='wf-muted'>Pipeline</small><br/><strong>3.2x</strong></div></div></main><aside class='wf-card' style='display:flex;flex-direction:column;gap:10px'><strong>Agent</strong><div class='wf-box'>Summarize Q3 versus Q2 and flag risks.</div><div class='wf-box' style='background:var(--wf-accent-soft)'>Revenue up 12%. Two pipeline risks flagged.</div><div style='flex:1'></div><div class='wf-card' style='padding:10px;display:flex;flex-direction:column;gap:8px'><span class='wf-muted'>Ask about this dashboard...</span><div style='display:flex;align-items:center;gap:6px'><span class='wf-pill'>Q3 Forecast</span><span class='wf-pill'>GPT-5</span><div style='flex:1'></div><button class='primary'><span data-icon='send'></span></button></div></div></aside></div>"
    }
  />
</WireframeBlock>

<Diagram id="doc-block-toolkits-boundary" title="So passen die drei Ebenen zusammen" summary={"Core ist die Framework-Grundlage. Toolkit bietet optionale wiederverwendbare Infrastruktur und UI. Templates sind optionale Apps auf Core, die häufig Toolkit nutzen."}>

```html
<div class="diagram-row">
  <div class="diagram-card">
    <span class="diagram-pill accent">Core</span><br />
    Aktionen, SQL, Authentifizierung, App-Zustand, Zugriffskontrollen,
    Echtzeittransport
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill accent">Toolkit</span><br />
    Zusammenarbeit, Teilen, Einrichtung, Review, Verlauf, Beobachtbarkeit
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill accent">Templates</span><br />
    Domänenschema, Routen, Aktionen, Marke, spezialisierte Abläufe
  </div>
</div>
```

```css
.diagram-row {
  display: flex;
  gap: 12px;
  align-items: stretch;
  flex-wrap: wrap;
}
.diagram-row .diagram-card {
  flex: 1 1 210px;
  padding: 14px;
}
.diagram-row .diagram-arrow {
  align-self: center;
  font-size: 22px;
}
```

</Diagram>

Core ist das Framework. Toolkit ist ein optionaler Vorrat wiederverwendbarer Framework-Bausteine. Templates sind optionale Apps auf Core; sie können Toolkit verwenden und als vollständige Ausgangspunkte geforkt werden. Eine App kann Core auch direkt ohne Toolkit oder Template nutzen.

## Katalog {#catalog}

| Bereich                     | Einstieg                                               | Inhalt                                                                             |
| --------------------------- | ------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| Grundlagen                  | [UI-Primitives](/docs/toolkit-ui)                      | Gemeinsame Controls, Hooks, App-Adapter und Shell-Nähte.                           |
| Editoren & Leinwände        | [Editoren & Leinwände](/docs/toolkit-editors-canvases) | Reichhaltige Bearbeitung, strukturierte Blöcke, Canvas-Controls und Design-Muster. |
| Agentenerlebnis & Kontext   | [Kontext & Wissen](/docs/toolkit-context-knowledge)    | Context X-Ray, Ressourcen, Wissen, Composer-Kontext und agentensichtbarer Zustand. |
| Zusammenarbeit & Governance | [Zusammenarbeit](/docs/toolkit-collaboration)          | Teilen, Präsenz, Review, Historie, Audit und Organisationsgrenzen.                 |
| Workspace-Chrome            | [App-Chrome](/docs/toolkit-settings)                   | Einstellungen, Teams, Setup, Navigation, Ressourcen und Agenten-UX.                |
| Betrieb                     | [Observability](/docs/toolkit-observability)           | Nutzung, Feedback, Replay, Diagnose und operative Sichtbarkeit.                    |
| Toolkit-Fähigkeitsmodule    | [Fähigkeitsmodule](/docs/toolkit-capability-packages)  | Bei Bedarf installierte Domänenpakete und ihre Lifecycle-Manifeste.                |

## Produkt-Kits {#product-kits}

| Kit                                                      | Verwende es für                                                                                                             |
| -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| [Sharing](/docs/toolkit-sharing)                         | Besitzbare Ressourcen, Teilen-Dialoge/-Aktionen, Sichtbarkeit und öffentlicher/privater/Org-Zugriff.                        |
| [Collaboration](/docs/toolkit-collaboration)             | Echtzeitbearbeitung, Präsenz, Live-Cursor, letzte Änderungen und gemeinsames Rückgängig.                                    |
| [Settings](/docs/toolkit-settings)                       | Standardseiten/-tabs für App- und Agent-/Provider-Einstellungen an einem vorhersehbaren Ort.                                |
| [Org & Team](/docs/toolkit-org-team)                     | Teamseiten, Org-Wechsel, Einladungen, Org-Domänen, Rollen und App-Wechsel.                                                  |
| [Setup & Connections](/docs/toolkit-setup-connections)   | Onboarding, Secrets, Workspace-Verbindungen, OAuth/Provider-Bereitschaft und Builder Connect (kostenloser Tarif verfügbar). |
| [Command & Navigation](/docs/toolkit-command-navigation) | Befehlsmenü, Deep Links, Standardrouten sowie Sidebar- und Shell-Muster.                                                    |
| [Resources](/docs/toolkit-resources)                     | Ressourcenbäume/-editoren, Dateiauswahl/-suche, Anhänge und lokaler Dateimodus.                                             |
| [Agent UX](/docs/toolkit-agent-ux)                       | Composer, Modellauswahl, Kontext-Chips, Freigaben, Wiederherstellung und Run-Leiste.                                        |
| [History](/docs/toolkit-history)                         | Versionierte Ressourcen, Snapshots, Verlauf, Audit, Undo/Redo, Wiederherstellung und Änderungen.                            |
| [Comments & Review](/docs/toolkit-comments-review)       | Prüfbare Ressourcen, Kommentare, Feedback-Warteschlangen, Threads, Erwähnungen und Review-Status.                           |
| [Observability](/docs/toolkit-observability)             | Nutzung, Ausgaben, Feedback, Replay, Fehler und Laufdiagnosen.                                                              |

## Paket-Reife und Ownership {#package-maturity-and-ownership}

- Der Rich-Text-Editor gehört zu **Toolkit**; vorhandene Core-Imports bleiben während der Migration schlanke Kompatibilitäts-Re-Exports.
- Design ist ein **Template-Muster** mit wiederverwendbaren `design-tweaks`, kein vollständiger Drop-in-Editor oder Canvas.
- Blocks und die Context-X-Ray-Engine bleiben in dieser Phase **Core-owned**; die wiederverwendbare Context-X-Ray-Darstellung gehört zu Toolkit.
- Creative Context, Scheduling und Pinpoint sind **Toolkit-Fähigkeitsmodule**, die bei Bedarf installiert werden; sie behalten separate npm-Pakete und Lifecycle-Manifeste. Dispatch bleibt ein separates Produkt.

## Dashboard-Kit {#dashboard-kit}

`@agent-native/toolkit/dashboard` ist eine auswerfbare reine Darstellungseinheit
für Kennzahlen- und Statistik-Karten, Tabellen, Datumsbereich, Panel-Reihenfolge
und -Layout sowie generische Charts. Die App liefert Zeilen und Callbacks; das
Kit lädt weder Daten noch verwaltet es Dashboard-Zustand oder Credentials.
Instanziiere `@agent-native/core/dashboard-storage` pro App mit eigenem Schema
und Zugriff; die Standardquelle `program` führt ein app-eigenes Datenprogramm aus.
Führe vor der Übernahme `agent-native eject inspect toolkit/dashboard` aus.
Provider-Resolver bleiben in der App, die die Verbindung und Abfragerichtlinie besitzt.

## Implementierungsgrenze {#implementation-boundary}

Kits sind zuerst Produktoberflächen. Manche Teile werden von der Laufzeit
(`@agent-native/core`), some are presentation primitives
(`@agent-native/toolkit`), and apps usually wrap both through local adapters.
Nutze den Paketpfad, dem das benötigte Verhalten gehört, aber gestalte
Einstellungen, Zusammenarbeit, Teilen, Team, Review und Einrichtung als ein
stimmiges Kit statt Paketgrenzen in der App sichtbar zu machen.

## Grundbausteine {#foundation-pieces}

- [UI-Primitives](/docs/toolkit-ui) behandelt die gemeinsamen shadcn-artigen
  Primitives unter `@agent-native/toolkit/ui/*`, lokale App-Adapter und
  gemeinsame Shell-Hooks.
- Die [Component API](/docs/components) bleibt die Referenz auf niedriger Ebene
  für öffentliche React-Exports.

## Designregel {#design-rule}

Verwende ein Kit, wenn Verhalten appübergreifend wiederkehrt und Nutzer einen
Standard erwarten: Teilen, Einstellungen, Teams, Kommentare, Verlauf und
Agentenfortschritt sollten nicht je Template anders sein. Domänendaten,
Abläufe, Labels und visuelles Styling bleiben in der App.

## Nächste Schritte

- [**Toolkit UI-Primitives**](/docs/toolkit-ui) — gemeinsame Primitives, App-Adapter und Shell-Hooks.
- [**Sharing Kit**](/docs/toolkit-sharing) — ein guter erster Einstieg für Ressourcen, Teilen und Sichtbarkeit.
- [**Templates erstellen**](/docs/creating-templates) — Templates Ende-zu-Ende bauen und Kits im Kontext sehen.
- [**Komponenten-API**](/docs/components) — die Referenz aller öffentlichen React-Exports.
- [**Paket-Lebenszyklus**](/docs/package-lifecycle) — manifestgesteuerte Toolkit-Fähigkeitsmodule sicher untersuchen, hinzufügen und auswerfen.

## Ein Kit anpassen oder auswerfen {#customize-or-eject}

Folgen Sie dieser Reihenfolge: öffentliche API konfigurieren → öffentliche Primitives zusammensetzen → kleinste Einheit auswerfen → gemeinsame Naht vorschlagen. Erkunden Sie mit `agent-native eject --list` und `agent-native eject inspect <unit>`; prüfen Sie danach `agent-native eject <unit> --app <app>` als Vorschau, bevor Sie `--apply` hinzufügen.

Toolkit-Manifeste decken jeden öffentlichen Einstiegspunkt ab. Die Anwendung protokolliert Quelle und Import-Umschreibungen in `agent-native.ejections.json`. `agent-native eject diff <unit> --app <app>` zeigt Abweichungen; `restore` läuft nur, wenn Hashes und Importe unverändert sind. First-Party-Einheiten decken auch Remote-MCP-Kataloge, Workspace-Verbindungen, Provider-API-Definitionen, Messaging-Adapter, Setup-/Readiness-UI und Domänenpakete ab. Ein fehlendes First-Party-Rezept lässt den Release-Guard fehlschlagen; Drittanbieter erhalten einen Add-Blueprint; geschützte Core-Verträge bleiben hinter öffentlichen Erweiterungsnähten.
