...
<Diagram id="doc-block-1o16dfy" title={"Flujo de federación de identidades"} summary={"Dispatch autentica al ser humano y devuelve una afirmación firmada de corta duración de una cosa: el correo electrónico verificado. La aplicación se vincula por correo electrónico y genera su propia sesión local."}>
...
4. **La aplicación verifica y enlaza JIT por correo electrónico.** La ruta `GET /_agent-native/identity/callback` verifica la firma del token con `A2A_SECRET`, comprueba `scope: "identity"` y `exp`, y realiza el **enlace justo a tiempo estrictamente por correo electrónico verificado**:
...
**Es seguro habilitarlo, es seguro revertirlo.** Las escrituras de identidad son **solo aditivas**: una cuenta de correo electrónico existente se reutiliza intacta y un correo electrónico nuevo simplemente inserta una fila nueva. No hay cambios de esquema ni nada que migrar, por lo que activar o desactivar `AGENT_NATIVE_IDENTITY_HUB_URL` es completamente reversible en cualquier momento, por aplicación.

</Callout>

El enlace justo a tiempo es una decisión única basada completamente en el correo electrónico verificado:

<Diagram id="doc-block-1q5bm3x" title={"JIT-link decisión"} summary={"La vinculación se ingresa en el correo electrónico verificado y es solo aditiva: las cuentas existentes se reutilizan sin cambios, los nuevos correos electrónicos crean un nuevo usuario local."}>
