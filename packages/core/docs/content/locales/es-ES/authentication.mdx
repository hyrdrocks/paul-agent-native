...
Cuánto tiempo se almacena en caché esa shell es una configuración de toda la implementación — consulta [SSR Caching](/docs/deployment#ssr-caching) para `AGENT_NATIVE_SSR_CACHE`. Acortar o desactivar la caché solo cambia la duración; el SSR sigue renderizando la rama anónima con las cookies eliminadas.
