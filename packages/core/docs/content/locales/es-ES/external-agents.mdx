...
El id de servidor de ese bloque (`mail` arriba) es `agent-native-<primera etiqueta de su nombre de host>` por defecto. Defina `MCP_SERVER_NAME` en el entorno del despliegue para elegirlo usted mismo: la página de conexión, la configuración copiable y las concesiones del flujo de dispositivo usan el mismo valor. Una app que pasa `mcpConnectServerName` a `createCoreRoutesPlugin` conserva ese nombre; la variable de entorno solo se aplica cuando la opción está ausente.
