...
## तीन परतें {#three-layers}

Agent Native एक framework है, कोई एक template नहीं:

- **Core - framework:** हर ऐप के लिए बुनियादी runtime और data contract।
- **Toolkit - वैकल्पिक पुन: उपयोग योग्य pieces:** साझा UI और product systems जिन्हें ऐप अपना, compose या fork कर सकते हैं।
- **Templates - वैकल्पिक apps:** Core पर बने पूर्ण domain apps, जो अक्सर Toolkit का उपयोग करते हैं और जिन्हें starting point के रूप में fork और उपयोग किया जा सकता है।

Core foundation है। Toolkit और Templates वैकल्पिक हैं।

## वास्तुकला {#the-architecture}

प्रत्येक एजेंट-नेटिव ऐप तीन चीजें एक साथ काम करती हैं:

- **एजेंट** - स्वायत्त एआई जो डेटा पढ़ता है, डेटा लिखता है, actions चलाता है और configured tools का उपयोग करता है। skills और निर्देशों के साथ अनुकूलन योग्य।
...
- **कंप्यूटर** — डेटाबेस, ब्राउज़र, कोड निष्पादन। एजेंट ऐप की एक्शन और डेटा सतह के माध्यम से काम करते हैं; ऐप उसी एक्शन सतह को MCP के ज़रिए उपलब्ध करा सकता है। बाहरी MCP सर्वर वैकल्पिक ऐड-ऑन हैं, फाउंडेशन नहीं।

<Diagram id="doc-block-1dvva76" title={"एजेंट, एप्लिकेशन और कंप्यूटर"} summary={"एक साझा SQL स्टोर पर तीन परतें एक साथ काम कर रही हैं। एजेंट और एप्लिकेशन दोनों समान डेटा पढ़ते और लिखते हैं।"}>
...
<div class="diagram-arch">
  <div class="diagram-row">
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >डेटा पढ़ता और लिखता है, actions चलाता है, configured tools का उपयोग
        करता है</small
      >
    </div>
    <div class="diagram-card">
      ...
      ><small class="diagram-muted"
        >सिर्फ action, chat, control plane, या full React इंटरफेस</small
      >
    </div>
  </div>
  ...
  <div class="diagram-box" data-rough>
    Computer<br /><small class="diagram-muted"
      >SQL डेटाबेस · browser · code execution</small
    >
  </div>
</div>
```
...
<FileTree
  id="doc-block-ieye0"
  title={"Guidance और behavior"}
  entries={[
    {
      path: "AGENTS.md",
      note: "always-on instructions: purpose, core rules, state keys, actions index, skills index",
    },
    {
      path: ".agents/skills/<name>/SKILL.md",
      note: "reusable behavior: workflow steps, policies, examples, references, और do/don’t lists",
    },
    {
      path: "actions/<name>.ts",
      note: "executable capability: agent, UI, CLI, HTTP, MCP, A2A, jobs, और webhooks को exposed typed operation",
    },
  ]}
/>
...
पाँच नियम वास्तुकला को नियंत्रित करते हैं:
...
5. **Application state SQL में है** - ephemeral UI state database में रहता है और agent तथा UI दोनों पढ़ सकते हैं
...
<DataModel
  ...
  title={"कोर SQL स्टोर"}
  summary={
    "प्रत्येक टेम्पलेट में स्वतः निर्मित - एजेंट और UI दोनों इन्हें पढ़ते और लिखते हैं।"
  }
  entities={[
    {
      id: "application_state",
      name: "application_state",
      note: "अल्पकालिक UI बताता है कि एजेंट संदर्भ के लिए पढ़ता है",
      fields: [
        {
          name: "key",
          type: "text",
    ...
    },
    {
      id: "settings",
      name: "settings",
      note: "लगातार कुंजी-मूल्य कॉन्फ़िगरेशन",
      fields: [
        {
          name: "key",
          type: "text",
...
```bash
# त्वरित डेटाबेस निरीक्षण के लिए मुख्य कार्रवाइयां
pnpm action db-schema                                       # show all tables
pnpm action db-query --sql "SELECT * FROM forms"
```

production agent chat plugin raw SQL tools को default रूप से read-only रखता है
(`frameworkTools: { database: "read" }`), इसलिए agent app data को `db-schema` / `db-query`
से inspect करते हैं और writes typed app actions से करते हैं। `database: "write"`
(या `true`) केवल उन maintenance surfaces के लिए सेट करें जिन्हें scoped
`db-exec` / `db-patch` भी चाहिए; सभी data access के लिए typed app actions
require करने हेतु `database: "off"` / `false` सेट करें
पहुँच.

`frameworkTools` फ़्रेमवर्क के बाकी अपने टूल को भी इसी तरह नियंत्रित करता है: sharing, रिव्यू कमेंट, वर्शन इतिहास, feature flags, लोकलाइज़ेशन, ऑडिट, Context X-Ray, प्रोफ़ाइल, ऑटोमेशन, docs, resources, web, ऐप-दर-ऐप डेलिगेशन, चैट और ईमेल। पूरी सूची, `"minimal"` preset, और किसी समूह को बंद करने पर उसके HTTP रूट क्यों माउंट रहते हैं — देखें [Production Agent Tools](/docs/deployment#production-agent-tools)।
...
<Diagram id="doc-block-maof0j" title={"लाइव सिंक प्रवाह"} summary={"एक एजेंट राइट बिना किसी मैन्युअल रिफ्रेश के यूआई रेंडर बन जाता है - SSE पहले, यूनिवर्सल फ़ॉलबैक के रूप में मतदान।"}>

```html
<div class="diagram-sync">
  <div class="diagram-node">
    एजेंट action<br /><small class="diagram-muted">DB में लिखता है</small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    बदलाव इवेंट<br /><small class="diagram-muted"
  ...
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-node">
    क्वेरी फिर से fetch<br /><small class="diagram-muted"
      >रेंडर, रीलोड नहीं</small
    >
  </div>
</div>
```
...
## ऐप कोड और अनुकूलन {#agent-modifies-code}

Embedded agent डिफ़ॉल्ट रूप से ऐप का source code संपादित नहीं करता। वह ऐसा
केवल तभी कर सकता है जब host जानबूझकर repository/workspace write tooling दे।
सामान्य तैनात ऐप में agent actions, SQL-backed state और configured integrations
के माध्यम से काम करता है। Templates पूर्ण apps हैं जिन्हें आप अपने repository
और development workflow में fork और customize कर सकते हैं।
...
- **[डेटा प्रोग्राम](/docs/data-programs)** - एजेंट द्वारा लिखी गई सहेजी गई `run-code` स्क्रिप्ट्स जो dashboard पैनलों को हार्डकोडेड provider action के बजाय एक कैश्ड, रिफ्रेश करने योग्य परिणाम देती हैं।
...
फ्रेमवर्क को अपनाना ज्यादातर इसलिए मूल्यवान है क्योंकि आपको निर्माण करना बंद कर देना चाहिए। जैसे ही आपका ऐप पाँच नियमों का पालन करता है, आपको यह विरासत में मिलता है:
...
- **प्रति उपयोगकर्ता एक पूर्ण कार्यक्षेत्र।** Skills, साझा `LEARNINGS.md`, व्यक्तिगत `memory/MEMORY.md`, `AGENTS.md`, कस्टम उप-एजेंट, निर्धारित नौकरियां, कनेक्टेड MCP सर्वर - सभी SQL-समर्थित, कोई डेव-बॉक्स आवश्यक नहीं है। [एजेंट संसाधन](/docs/agent-resources) देखें.
...
- [**What Is Agent-Native?**](/docs/what-is-agent-native) — इन नियमों के पीछे की दृष्टि और दर्शन
- [**Context Awareness**](/docs/context-awareness) — navigation state, view-screen और navigate commands का विस्तृत विवरण
- [**Skills Guide**](/docs/skills-guide) — framework skills, domain skills और custom skills बनाना
- [**Native Chat UI**](/docs/native-chat-ui) — action-declared tables, charts और अपना runtime लाने का तरीका
- [**Agent Surfaces**](/docs/agent-surfaces) — chat, native inline UI, पूर्ण app pages, embedded sidecar, automation और external-agent paths
- [**A2A Protocol**](/docs/a2a-protocol) — agent-to-agent संचार
