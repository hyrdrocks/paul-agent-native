...
셸이 캐시되는 기간은 배포 전체 설정입니다 — `AGENT_NATIVE_SSR_CACHE`에 대해서는 [SSR Caching](/docs/deployment#ssr-caching)을 참조하세요. 캐시를 단축하거나 비활성화해도 기간만 바뀔 뿐, SSR은 여전히 쿠키가 제거된 익명 분기를 렌더링합니다.
