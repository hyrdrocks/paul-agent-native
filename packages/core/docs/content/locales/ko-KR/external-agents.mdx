...
### 앱 내 Access 탭 {#access-tab}

`AgentTabsPage`를 마운트하는 앱은 **Access** 탭에서 동일한 연결 흐름을
제공합니다. MCP URL, 가능한 경우 A2A 에이전트 카드, Claude, ChatGPT, Cursor,
Claude Code, Codex 및 기타 클라이언트를 위한 공통 설정 가이드를 보여줍니다.
앱 안에서 시작점으로 사용하고 전체 흐름과 토큰 대체 경로는 `/mcp/connect`에서
여세요. 이 탭은 연결 설정 UI이지 일반 접근 제어 콘솔이 아니며, grants, 범위
편집, 취소는 아직 관리하지 않습니다.
...
해당 블록의 서버 ID(위의 `mail`)는 기본적으로 `agent-native-<호스트 이름의 첫 번째 라벨>`입니다. 직접 지정하려면 배포 환경에 `MCP_SERVER_NAME`을 설정하세요. 연결 페이지, 복사 가능한 구성, 장치 흐름 부여가 모두 동일한 값을 사용합니다. `createCoreRoutesPlugin`에 `mcpConnectServerName`을 전달하는 앱은 해당 이름을 유지합니다. 환경 변수는 옵션이 없을 때만 적용됩니다.
