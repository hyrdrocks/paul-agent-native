...
<Diagram id="doc-block-toolkits-boundary" title="세 계층의 관계" summary={"Core는 프레임워크 기반입니다. Toolkit은 선택적 재사용 인프라와 UI입니다. Templates는 Core 위에 구축되고 흔히 Toolkit을 사용하는 선택적 앱입니다."}>
...
Core가 프레임워크입니다. Toolkit은 프레임워크를 위한 선택적 재사용 부품 모음입니다. Templates는 Core 위의 선택적 앱이며 Toolkit을 사용할 수 있고 완전한 시작점으로 포크할 수 있습니다. 앱은 Toolkit이나 template 없이 Core를 직접 사용할 수도 있습니다.
...
| [설정 및 연결](/docs/toolkit-setup-connections)  | 온보딩, 시크릿, 워크스페이스 연결, OAuth/프로바이더 준비 상태, Builder 연결(무료 요금제 제공). |
...
## 대시보드 키트 {#dashboard-kit}

`@agent-native/toolkit/dashboard`는 지표·통계 카드, 테이블, 날짜 범위, 패널 순서와
레이아웃, 범용 차트를 위한 표현 전용 eject 가능 단위입니다. 앱이 행과 콜백을
제공하며 키트는 데이터를 가져오거나 대시보드 상태 또는 자격 증명을 소유하지
않습니다. `@agent-native/core/dashboard-storage`는 앱별 스키마와 접근 제어로
인스턴스화하고, 기본 `program` source는 앱 소유 data program을 실행합니다.
적용하기 전에 `agent-native eject inspect toolkit/dashboard`를 실행하세요.
provider resolver는 연결과 query policy를 소유한 앱에 둡니다.
...
## Kit 사용자 지정 또는 꺼내기 {#customize-or-eject}

공개 API 구성 → 공개 프리미티브 조합 → 가장 작은 단위 꺼내기 → 공유 seam 제안 순서로 진행합니다. `agent-native eject --list`와 `agent-native eject inspect <unit>`으로 살펴보고, `--apply`를 추가하기 전에 `agent-native eject <unit> --app <app>`을 미리 봅니다.

Toolkit 매니페스트는 모든 공개 진입점을 포함합니다. 앱은 source와 import 재작성을 `agent-native.ejections.json`에 기록합니다. `agent-native eject diff <unit> --app <app>`은 drift를 보여 주며 hash와 import가 그대로일 때만 `restore`가 실행됩니다. First-party unit은 remote MCP catalog, workspace connection, provider API definition, messaging adapter, setup/readiness UI, domain package도 포함합니다. First-party recipe가 빠지면 release guard가 실패합니다. 외부 package에는 add blueprint를 제공하고 보호된 Core contract는 공개 extension seam 뒤에 유지합니다.
