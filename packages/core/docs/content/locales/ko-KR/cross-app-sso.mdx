...
## 사용자가 보는 화면 {#user-flow}

Cross-App SSO는 두 번째 계정을 만드는 것이 아니라 하나의 워크스페이스에서 제품 사이를 이동하는 것처럼 느껴져야 합니다. 예를 들면 다음과 같습니다.

1. [Dispatch](https://dispatch.agent-native.com) 또는 워크스페이스 ID를 이미 사용하는 앱에 로그인합니다.
2. [Mail](https://mail.agent-native.com), [Calendar](https://calendar.agent-native.com) 또는 활성화된 다른 앱을 새 탭에서 엽니다.
3. Dispatch 세션이 없을 때만 앱이 브라우저를 Dispatch로 보냅니다. 세션이 있으면 리디렉션은 사실상 보이지 않습니다.
4. 같은 이메일, 레코드, 조직 컨텍스트를 유지한 채 이미 로그인된 앱으로 돌아옵니다.

중요한 차이는 이것이 **공유 쿠키가 아니라 한 번의 로그인**이라는 점입니다. 별도 호스트 앱은 계속 자체 로컬 세션을 만들고 관리합니다. Dispatch가 앱의 입구에서 사용자의 ID를 증명하면, 앱은 자체 인증과 일반적인 접근 검사를 계속 적용합니다.

제품 데모에서는 이 순서를 그대로 녹화하세요. Dispatch에 한 번 로그인하고, 다른 호스트 앱 두 개를 열어 어느 앱도 자격 증명을 다시 요청하지 않는 모습을 보여준 다음, 한 앱에서 로그아웃하여 다른 앱의 세션이 독립적임을 보여줍니다.
...
<Diagram id="doc-block-1axbtj8" title={"ID 페더레이션 흐름"} summary={"Dispatch은 사람을 인증하고 확인된 이메일이라는 단기간 서명된 주장을 반환합니다. 앱은 이메일로 연결되며 자체 로컬 세션을 시작합니다."}>
...
<div class="diagram-sso">
  ...
  <div class="diagram-card" data-rough>
    ...
    ><span class="diagram-pill ok">이메일 기반 JIT 연결</span
...
1. **앱 → Dispatch(승인).** 앱의 `GET /_agent-native/identity/login` 경로(“Agent-Native로 로그인” 버튼이 연결되는 경로)가 리디렉션을 구성하고 사용자를 ID 기관으로 보냅니다.
   ...
   `redirect_uri`는 이 앱의 `GET /_agent-native/identity/callback` 경로이며, 3단계에서 서명된 토큰을 받는 경로입니다.

<Endpoint id="cross-app-sso-identity-authorize-ko-kr" title={"ID 인증 엔드포인트"} method="GET" path="/_agent-native/identity/authorize" summary={"디스패치(ID 기관)는 사람을 인증하고 서명된 ID 토큰으로 다시 리디렉션합니다."} auth={"디스패치 세션(없는 경우 대화형 로그인)"} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "요청하는 앱 식별자입니다."
  },
  {
    "name": "redirect_uri",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "앱 콜백 URL. 엄격한 허용 목록(기본적으로 `*.agent-native.com` 또는 localhost)에 대해 검증되었습니다."
  },
  {
    "name": "state",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "CSRF 상태가 리디렉션 시 다시 에코되었습니다."
  }
]} responses={[
  {
    "status": "302",
    "description": "단기 `A2A_SECRET` 서명 ID JWT(`scope: \"identity\"`, `exp` ≤ 2분)와 원본 `state`를 전달하는 `redirect_uri`로 리디렉션합니다."
  },
  {
    "status": "400",
    "description": "`redirect_uri`가 허용 목록 유효성 검사에 실패했습니다(교차 출처, 스키마 기준 `//host` 또는 목록에 없는 접미사)."
  }
]}>
...
4. **앱은 이메일로 확인하고 JIT 연결합니다.** 앱의 `GET /_agent-native/identity/callback` 경로는 자체 `A2A_SECRET`로 토큰 서명을 확인하고 `scope: "identity"`와 `exp`를 검사한 다음 **확인된 이메일만으로 적시 연결**을 수행합니다.
...
<Callout id="doc-block-1ccuipn" tone="success">

**활성화해도 안전하고 되돌려도 안전합니다.**신원 쓰기는**추가만 가능합니다**— 기존의 동일한 이메일 계정은 그대로 재사용되며 새 이메일은 새로운 행을 삽입합니다. 스키마 변경이나 마이그레이션할 사항이 없으므로 `AGENT_NATIVE_IDENTITY_HUB_URL` 켜기 또는 끄기를 앱별로 언제든지 완전히 되돌릴 수 있습니다.

</Callout>

적시 링크는 확인된 이메일에 전적으로 기반한 단일 결정입니다.

<Diagram id="doc-block-9m3ozw" title={"JIT-link 결정"} summary={"연결은 확인된 이메일에 입력되며 추가로만 이루어집니다. 기존 계정은 변경 없이 재사용되고, 새 이메일은 새로운 로컬 사용자를 생성합니다."}>

```html
<div class="diagram-jit">
  <div class="diagram-node" data-rough>
    확인된 이메일<br /><small class="diagram-muted"
      >서명된 identity JWT에서</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-branch">
    ...
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>일반 로컬 세션 발급</div>
  </div>
</div>
```
