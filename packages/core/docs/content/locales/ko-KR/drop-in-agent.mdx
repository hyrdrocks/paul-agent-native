...
- **`type`** — `"content"`(기본값)는 앱 도구를 사용하는 임베디드 에이전트에서 작업을 유지합니다. `"code"`는 저장소 소스 변경을 위한 별도의 코드 지원 프레임으로 라우팅합니다. 쓰기는 호스트가 repository/workspace 도구를 의도적으로 부여한 경우에만 사용할 수 있습니다. [Frames](/docs/frames)를 참조하세요.
