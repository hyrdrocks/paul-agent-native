...
## O que o usuário vê {#user-flow}

O Cross-App SSO deve parecer uma mudança entre produtos do mesmo espaço de trabalho, não a criação de uma segunda conta. Por exemplo:

1. Faça login no [Dispatch](https://dispatch.agent-native.com) ou em qualquer aplicativo que já use a identidade do espaço de trabalho.
2. Abra o [Mail](https://mail.agent-native.com), o [Calendar](https://calendar.agent-native.com) ou outro aplicativo habilitado em uma nova aba.
3. O aplicativo envia o navegador ao Dispatch somente se não houver uma sessão do Dispatch. Se a sessão já existir, o redirecionamento fica praticamente invisível.
4. Você retorna ao aplicativo já conectado, com o mesmo e-mail, registros e contexto da organização.

A distinção importante é que isso é **um login, não um cookie compartilhado**. Cada aplicativo hospedado separadamente continua criando e controlando sua própria sessão local. O Dispatch comprova a identidade do usuário na entrada do aplicativo, que então continua usando sua autenticação local e suas verificações de acesso normais.

Para uma demonstração do produto, grave exatamente esta sequência: faça login uma vez no Dispatch, abra dois outros aplicativos hospedados, mostre que nenhum deles pede credenciais novamente e depois saia de um para mostrar que a sessão do outro é independente.
...
<Diagram id="doc-block-v0atqe" title={"Fluxo de federação de identidade"} summary={"Dispatch autentica o humano e retorna uma declaração assinada de curta duração de uma coisa – o e-mail verificado. O aplicativo é vinculado por e-mail e cria sua própria sessão local."}>
...
<div class="diagram-sso">
  ...
  <div class="diagram-card" data-rough>
    <strong>Callback do app</strong
    ><small class="diagram-muted"
      >verify signature · scope:identity · exp &le; 2 min</small
    ><span class="diagram-pill ok">Vinculação JIT por e-mail</span
...
1. **Aplicativo → Dispatch (autorização).** A rota `GET /_agent-native/identity/login` do aplicativo (à qual o botão “Entrar com Agent-Native” aponta) cria o redirecionamento e envia o usuário à autoridade de identidade:
   ...
   `redirect_uri` é a rota `GET /_agent-native/identity/callback` do próprio aplicativo — a rota que recebe o token assinado na etapa 3.

<Endpoint id="cross-app-sso-identity-authorize-pt-br" title={"Endpoint de autorização de identidade"} method="GET" path="/_agent-native/identity/authorize" summary="Dispatch (autoridade de identidade) autentica o humano e redireciona de volta com um token de identidade assinado" auth={"Sessão de envio (login interativo se não houver)"} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "O identificador do aplicativo solicitante."
  },
  {
    "name": "redirect_uri",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "Retorno de chamada do aplicativo URL. Validado em uma lista de permissões estrita (`*.agent-native.com` ou localhost por padrão)."
  },
  {
    "name": "state",
    "in": "query",
    "type": "string",
    "required": true,
    "description": "Estado CSRF ecoado de volta no redirecionamento."
  }
]} responses={[
  {
    "status": "302",
    "description": "Redireciona para `redirect_uri` carregando uma identidade JWT assinada por `A2A_SECRET` de curta duração (`scope: \"identity\"`, `exp` ≤ 2 minutos) mais o `state` original."
  },
  {
    "status": "400",
    "description": "`redirect_uri` falhou na validação da lista de permissões (origem cruzada, `//host` relativo ao esquema ou sufixo não listado)."
  }
]}>
...
4. **O aplicativo verifica e vincula JIT por e-mail.** A rota `GET /_agent-native/identity/callback` verifica a assinatura do token com `A2A_SECRET`, confere `scope: "identity"` e `exp` e realiza **vinculação just-in-time estritamente por e-mail verificado**:
...
<div class="diagram-jit">
  ...
  <div class="diagram-branch">
    ...
    <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
    <div class="diagram-box" data-rough>Emitir sessão local normal</div>
  </div>
</div>
```
