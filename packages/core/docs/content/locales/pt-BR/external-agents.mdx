...
### A aba Access no aplicativo {#access-tab}

Os aplicativos que montam `AgentTabsPage` oferecem o mesmo fluxo de conexão na
aba **Access**. Ela mostra a URL MCP, um cartão de agente A2A quando disponível e
os guias compartilhados de configuração para Claude, ChatGPT, Cursor, Claude
Code, Codex e outros clientes. Use-a como ponto de partida dentro do app e abra
`/mcp/connect` para o fluxo completo e o fallback de token. É uma UI de
configuração de conexão, não um console geral de acesso: a aba ainda não gerencia
grants, edição de escopo ou revogação.
...
O id do servidor nesse bloco (`mail` acima) é `agent-native-<primeiro rótulo do seu nome de host>` por padrão. Defina `MCP_SERVER_NAME` no ambiente do deploy para escolhê-lo você mesmo — a página de conexão, a configuração copiável e as concessões do fluxo de dispositivo usam o mesmo valor. Um app que passa `mcpConnectServerName` para `createCoreRoutesPlugin` mantém esse nome; a variável de ambiente só se aplica quando a opção está ausente.
