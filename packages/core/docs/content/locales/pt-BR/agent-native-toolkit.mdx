---
title: "Toolkit"
description: "Um catálogo equilibrado de fundações reutilizáveis, editores e telas, contexto de agente, governança, estrutura do espaço de trabalho, operações e módulos de capacidade do Toolkit."
---

<!-- i18n-docs-ignore -->

# Toolkit

Toolkit é a prateleira de blocos reutilizáveis e testados em produção para apps
agênticos de qualidade. Ele transforma o trabalho de produto repetido entre apps
em superfícies padrão, sem colocar um padrão de app no centro do catálogo.

Esta seção é a prateleira única para descobrir UI reutilizável. Uma superfície
entra nestes documentos porque é reutilizável, mesmo quando Core ou um template
a possui hoje. Rótulos de propriedade e caminhos de importação deixam esse
limite claro sem espalhar a orientação de UI pelo site.

Each kit packages the UI patterns, action contracts, app-state expectations,
access boundaries, and agent-visible behavior that users expect to work
consistently. Apps still own their domain model and visual identity; Toolkit
make the common product systems feel complete from the first version instead of
being rebuilt in slightly different ways every time.

<WireframeBlock id="doc-block-toolkits-index">
  <Screen
    surface="desktop"
    html={
      "<div style='min-height:560px;box-sizing:border-box;display:grid;grid-template-columns:200px 1fr 280px;gap:16px'><aside class='wf-card' style='display:flex;flex-direction:column;gap:8px;padding:14px'><div style='display:flex;align-items:center;gap:8px'><div style='width:26px;height:26px;border-radius:8px;background:var(--wf-accent-soft);display:flex;align-items:center;justify-content:center;font-size:12px;flex:none'>A</div><strong style='flex:1'>Acme</strong><span data-icon='chevronDown'></span></div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px;border-radius:8px;background:var(--wf-accent-soft)'><span data-icon='search'></span> Home</div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px'><span data-icon='edit'></span> Documents</div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px'><span data-icon='user'></span> Team</div><div style='display:flex;align-items:center;gap:8px;padding:7px 9px'><span data-icon='settings'></span> Settings</div><div style='flex:1'></div><div style='display:flex;align-items:center;gap:8px;padding-top:10px;border-top:1.4px solid var(--wf-line)'><div style='width:24px;height:24px;border-radius:999px;background:var(--wf-accent-soft);flex:none'></div><small style='flex:1'>Steve</small><span data-icon='dots'></span></div></aside><main class='wf-card' style='display:flex;flex-direction:column;gap:14px'><div style='display:flex;align-items:center;gap:10px'><h2 style='margin:0'>Q3 Forecast</h2><div style='flex:1'></div><div style='display:flex'><div style='width:26px;height:26px;border-radius:999px;background:var(--wf-accent-soft);border:2px solid var(--wf-card);flex:none'></div><div style='width:26px;height:26px;border-radius:999px;background:var(--wf-accent-soft);border:2px solid var(--wf-card);margin-left:-8px;flex:none'></div><div style='width:26px;height:26px;border-radius:999px;background:var(--wf-accent);border:2px solid var(--wf-card);margin-left:-8px;flex:none'></div></div><button class='primary'><span data-icon='user'></span> Share</button></div><div class='wf-box' style='display:flex;flex-direction:column;gap:10px'><small class='wf-muted'>Revenue by month</small><div style='display:flex;align-items:flex-end;gap:10px;height:120px'><div style='flex:1;height:45%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:65%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:55%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:80%;background:var(--wf-accent-soft);border-radius:4px'></div><div style='flex:1;height:95%;background:var(--wf-accent);border-radius:4px'></div></div></div><div style='display:flex;gap:10px'><div class='wf-box' style='flex:1'><small class='wf-muted'>ARR</small><br/><strong>$4.2M</strong></div><div class='wf-box' style='flex:1'><small class='wf-muted'>Win rate</small><br/><strong>28%</strong></div><div class='wf-box' style='flex:1'><small class='wf-muted'>Pipeline</small><br/><strong>3.2x</strong></div></div></main><aside class='wf-card' style='display:flex;flex-direction:column;gap:10px'><strong>Agent</strong><div class='wf-box'>Summarize Q3 versus Q2 and flag risks.</div><div class='wf-box' style='background:var(--wf-accent-soft)'>Revenue up 12%. Two pipeline risks flagged.</div><div style='flex:1'></div><div class='wf-card' style='padding:10px;display:flex;flex-direction:column;gap:8px'><span class='wf-muted'>Ask about this dashboard...</span><div style='display:flex;align-items:center;gap:6px'><span class='wf-pill'>Q3 Forecast</span><span class='wf-pill'>GPT-5</span><div style='flex:1'></div><button class='primary'><span data-icon='send'></span></button></div></div></aside></div>"
    }
  />
</WireframeBlock>

<Diagram id="doc-block-toolkits-boundary" title="Como as três camadas se encaixam" summary={"Core é a base do framework. Toolkit oferece infraestrutura e UI reutilizáveis opcionais. Templates são apps opcionais sobre Core que normalmente usam Toolkit."}>

```html
<div class="diagram-row">
  <div class="diagram-card">
    <span class="diagram-pill accent">Core</span><br />
    ações, SQL, autenticação, estado do app, verificações de acesso, transporte
    em tempo real
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill accent">Toolkit</span><br />
    colaboração, compartilhamento, configuração, revisão, histórico,
    observabilidade
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-card">
    <span class="diagram-pill accent">Templates</span><br />
    esquema de domínio, rotas, ações, marca, fluxos especializados
  </div>
</div>
```

```css
.diagram-row {
  display: flex;
  gap: 12px;
  align-items: stretch;
  flex-wrap: wrap;
}
.diagram-row .diagram-card {
  flex: 1 1 210px;
  padding: 14px;
}
.diagram-row .diagram-arrow {
  align-self: center;
  font-size: 22px;
}
```

</Diagram>

Core é o framework. Toolkit é uma coleção opcional de peças reutilizáveis para o framework. Templates são apps opcionais construídos sobre Core; podem usar Toolkit e ser bifurcados como pontos de partida completos. Um app também pode usar Core diretamente sem Toolkit ou template.

## Catálogo {#catalog}

| Área                             | Comece aqui                                                | O que cobre                                                                               |
| -------------------------------- | ---------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Fundamentos                      | [Primitivos de UI](/docs/toolkit-ui)                       | Controles, hooks, adaptadores de app e integrações de shell compartilhados.               |
| Editores e telas                 | [Editores e telas](/docs/toolkit-editors-canvases)         | Edição rica, blocos estruturados, controles de tela e padrões de Design.                  |
| Experiência e contexto do agente | [Contexto e conhecimento](/docs/toolkit-context-knowledge) | Context X-Ray, recursos, conhecimento, contexto do compositor e estado visível ao agente. |
| Colaboração e governança         | [Colaboração](/docs/toolkit-collaboration)                 | Compartilhamento, presença, revisão, histórico, auditoria e limites organizacionais.      |
| Estrutura do espaço de trabalho  | [Chrome do app](/docs/toolkit-settings)                    | Configurações, equipes, setup, navegação, recursos e UX do agente.                        |
| Operações                        | [Observabilidade](/docs/toolkit-observability)             | Uso, feedback, replay, diagnóstico e visibilidade operacional.                            |
| Módulos de capacidade do Toolkit | [Pacotes de capacidade](/docs/toolkit-capability-packages) | Pacotes de domínio instalados sob demanda e seus manifestos de ciclo de vida.             |

## Kits de produto {#product-kits}

| Kit                                                      | Use para                                                                                                                |
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
| [Sharing](/docs/toolkit-sharing)                         | Recursos próprios, diálogos/ações de compartilhar, visibilidade e acesso público/privado/org.                           |
| [Collaboration](/docs/toolkit-collaboration)             | Edição em tempo real, presença, cursores ao vivo, mudanças recentes e desfazer colaborativo.                            |
| [Settings](/docs/toolkit-settings)                       | Páginas e abas padrão para configurações do app e do agente/provedor no mesmo lugar.                                    |
| [Org & Team](/docs/toolkit-org-team)                     | Páginas de equipe, troca de organização, convites, domínios, papéis e troca de app.                                     |
| [Setup & Connections](/docs/toolkit-setup-connections)   | Onboarding, segredos, conexões de workspace, OAuth/prontidão de provedor e Builder connect (nível gratuito disponível). |
| [Command & Navigation](/docs/toolkit-command-navigation) | Menu de comandos, links profundos, rotas padrão, padrões de barra lateral e shell.                                      |
| [Resources](/docs/toolkit-resources)                     | Árvores/editores de recursos, seletor/busca de arquivos, anexos e modo de arquivo local.                                |
| [Agent UX](/docs/toolkit-agent-ux)                       | Compositor, seletor de modelo, chips de contexto, aprovações, recuperação e gaveta de execuções.                        |
| [History](/docs/toolkit-history)                         | Recursos versionados, snapshots, histórico, auditoria, desfazer/refazer, restauração e mudanças.                        |
| [Comments & Review](/docs/toolkit-comments-review)       | Recursos revisáveis, comentários, feedback, threads, menções, resolução e status de revisão.                            |
| [Observability](/docs/toolkit-observability)             | Uso, gastos, feedback, replay, erros e diagnósticos de execução.                                                        |

## Maturidade e propriedade de pacotes {#package-maturity-and-ownership}

- O editor rico pertence ao **Toolkit**; imports Core existentes permanecem reexports de compatibilidade leves durante a migração.
- Design é um **padrão de template** com `design-tweaks` reutilizáveis, não um editor ou tela completa pronta para uso.
- Blocks e o mecanismo Context X-Ray permanecem propriedade do **Core** nesta fase; a apresentação reutilizável de Context X-Ray pertence ao Toolkit.
- Creative Context, Scheduling e Pinpoint são **módulos de capacidade do Toolkit** instalados sob demanda; mantêm pacotes npm e manifestos de ciclo de vida separados. Dispatch permanece um produto separado.

## Kit de dashboard {#dashboard-kit}

`@agent-native/toolkit/dashboard` é uma unidade ejetável só de apresentação:
cards de métricas e estatísticas, tabelas, intervalo de datas, ordem e layout
de painéis e gráficos genéricos. O app fornece linhas e callbacks; o kit não
busca dados nem mantém estado do dashboard ou credenciais. Instancie
`@agent-native/core/dashboard-storage` por app com esquema e acesso próprios;
a fonte padrão `program` executa um programa de dados do app.
Antes de aplicar, execute `agent-native eject inspect toolkit/dashboard`.
Os resolvedores de provedor permanecem no app que possui a conexão e a política de consulta.

## Limite de implementação {#implementation-boundary}

Kits são primeiro superfícies de produto. Algumas partes usam o runtime
(`@agent-native/core`), some are presentation primitives
(`@agent-native/toolkit`), and apps usually wrap both through local adapters.
Use o caminho do pacote que possui o comportamento necessário, mas projete
configurações, colaboração, compartilhamento, equipe, revisão e setup como um
kit coerente, sem expor limites de pacotes no app.

## Peças fundamentais {#foundation-pieces}

- [Primitivos de UI](/docs/toolkit-ui) cobre primitivos compartilhados no estilo
  shadcn em `@agent-native/toolkit/ui/*`, adaptadores locais e hooks de shell.
- A [API de componentes](/docs/components) continua sendo a referência de baixo
  nível para exports React públicos.

## Regra de design {#design-rule}

Use um kit quando o comportamento se repete entre apps e as pessoas esperam uma
experiência padrão: compartilhamento, configurações, equipes, comentários,
histórico e progresso do agente não devem variar entre templates. Mantenha no
app os dados, fluxos, rótulos e estilo visual específicos do domínio.

## Próximos passos

- [**Primitivos de UI do Toolkit**](/docs/toolkit-ui) — primitivos compartilhados, adaptadores de app e hooks de shell.
- [**Kit de compartilhamento**](/docs/toolkit-sharing) — um bom primeiro kit para recursos, diálogos de compartilhamento e visibilidade.
- [**Criar templates**](/docs/creating-templates) — crie um template completo e veja onde os kits se encaixam.
- [**API de componentes**](/docs/components) — a referência de todos os exports públicos do React.
- [**Ciclo de vida de pacotes**](/docs/package-lifecycle) — inspecione, adicione e ejete módulos de capacidade do Toolkit guiados por manifesto com segurança.

## Personalizar ou ejetar um kit {#customize-or-eject}

Siga esta ordem: configure a API pública → componha primitivos públicos → ejete a menor unidade → proponha uma interface compartilhada. Explore com `agent-native eject --list` e `agent-native eject inspect <unit>`; depois visualize `agent-native eject <unit> --app <app>` antes de adicionar `--apply`.

Os manifestos do Toolkit cobrem todos os pontos de entrada públicos. O app registra origem e reescritas de imports em `agent-native.ejections.json`. `agent-native eject diff <unit> --app <app>` mostra desvios; `restore` só funciona se hashes e imports estiverem intactos. As unidades first-party também cobrem catálogos MCP remotos, conexões de workspace, definições de provider API, adaptadores de mensagens, UI de configuração/prontidão e pacotes de domínio. Uma receita first-party ausente falha o guard de publicação; pacotes externos recebem um blueprint add; contratos protegidos do Core permanecem atrás de interfaces públicas de extensão.
