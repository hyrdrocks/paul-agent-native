...
### 应用内的 Access 选项卡 {#access-tab}

挂载 `AgentTabsPage` 的应用也会在 **Access** 选项卡中提供相同的连接流程。
它显示 MCP URL、可用时的 A2A 代理卡片，以及 Claude、ChatGPT、Cursor、Claude
Code、Codex 和其他客户端的共享设置指南。可将它作为应用内的起点，并打开
`/mcp/connect` 查看完整流程和令牌回退。这是连接设置界面，不是通用访问控制
台；该选项卡目前还不管理 grants、范围编辑或撤销。
...
该块中的服务器 id（上面的 `mail`）默认为 `agent-native-<主机名的第一个标签>`。在部署环境中设置 `MCP_SERVER_NAME` 即可自行指定 —— 连接页面、可复制的配置和设备流程授权都使用同一个值。向 `createCoreRoutesPlugin` 传入 `mcpConnectServerName` 的应用会保留该名称；仅当未提供该选项时才使用环境变量。
