...
您不需要从头开始构建原生代理。代理聊天、代理资源选项卡、CLI 终端、语音输入和所有相关基础设施都作为少数 React 组件提供给您放入任何应用程序中。
...
- **`type`** — `"content"`（默认）将工作保留在使用应用工具的嵌入式代理中。`"code"` 会路由到单独的代码能力 frame 以更改代码库源文件；只有当主机有意授予 repository/workspace 工具时才提供写入权限。参见 [Frames](/docs/frames)。
