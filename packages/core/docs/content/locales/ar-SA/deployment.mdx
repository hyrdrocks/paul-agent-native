...
### إعدادات حساسة للإنتاج {#env-required-prod}

تغطي هذه الإعدادات السلوك الحساس للإنتاج. يحتاج التطبيق المستقل الذي يستخدم مصادقة إطار العمل إلى سر `BETTER_AUTH_SECRET` ثابت وقاعدة بيانات دائمة؛ ويستنتج إطار العمل عنوان المصادقة العام من سياق الطلب أو القالب أو منصة الاستضافة عند عدم ضبطه. إعدادات مساحة العمل وA2A مشروطة بالميزات المستخدمة. القيم المفقودة إما مغلقة بشكل فاشل أو تتراجع إلى سلوك أضعف مع تحذير عالٍ.
...
| `BETTER_AUTH_URL`                           | تجاوز اختياري للأصل العام لهذا التطبيق (مثل `https://mail.example.com`). يستنتجه إطار العمل من سياق الطلب والقالب ومنصة الاستضافة عند عدم ضبطه.                                                                                               |
