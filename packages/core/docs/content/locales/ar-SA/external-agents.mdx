...
<Diagram id="doc-block-13sglel" title={"رحلة ذهابًا وإيابًا للوكيل الخارجي"} summary={"يقوم مضيف خارجي باستدعاء أداة عبر MCP؛ يقوم التطبيق بإرجاع قطعة أثرية بالإضافة إلى رابط مفتوح. يؤدي النقر فوقه إلى حل جلسة المتصفح وتركيز العنصر في واجهة المستخدم قيد التشغيل - لا يحمل الرابط أي حالة مميزة."}>

```html
<div class="xa-trip">
  <div class="diagram-box" data-rough>
    مضيف خارجي<br /><small class="diagram-muted"
      >Claude &middot; ChatGPT &middot; Codex &middot; Cursor</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
    <span class="diagram-pill accent">أداة MCP call</span
    ><small class="diagram-muted">e.g. <code>manage-draft</code></small>
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-box" data-rough>
    ينتج التطبيق أثرًا<br /><small class="diagram-muted"
      >+ <code>Open in &lt;app&gt; &rarr;</code> deep link / MCP App</small
    >
  </div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&darr;</div>
  <div class="diagram-box" data-rough>ينقر المستخدم على الرابط</div>
  <div class="diagram-arrow diagram-muted" aria-hidden="true">&rarr;</div>
  <div class="diagram-panel center" data-rough>
...
- **الوكلاء الفرعيون المحليون المخصصون:** استخدم [موارد الوكيل](/docs/agent-resources) عندما تريد ملفات تعريف الوكيل المخصصة داخل مساحة العمل الأصلية للوكيل نفسها.
...
يبحث `reconnect` عن أي إدخال تكوين MCP ينتهي URL بـ `/mcp` للمضيف المحدد والعميل المحدد (المطابقة بواسطة URL بغض النظر عن اسم الموصل)، ثم يقوم بتحديث مادة المصادقة أو استبدالها دون لمس skills المثبت أو إعادة تشغيل تدفق التثبيت الكامل. تمرير التطبيق الأساسي URL (على سبيل المثال، `https://plan.agent-native.com`) - يتم استنتاج لاحقة `/mcp`. يتم تحميل المصادقة والأداة لكل عميل، لذا قم بإعادة تشغيل/إعادة تحميل هذا العميل بعد ذلك؛ يحتاج Codex إلى جلسة جديدة قبل ظهور الأدوات المحملة حديثًا.
...
بالنسبة لعملاء MCP الذين لا يمكنهم إضافة OAuth URL عن بعد مباشرةً، افتح التطبيق في متصفحك واستخدم تكلفة **الاتصال** (المقدمة في `https://&lt;app&gt;/mcp/connect`). أثناء تسجيل الدخول، انقر فوق **الاتصال / التفويض**. تمنحك الصفحة إما رابطًا عميقًا بنقرة واحدة يقوم بتكوين الوكيل المكتشف، أو كتلة `.mcp.json` جاهزة للصقه:
...
معرّف الخادم في تلك الكتلة (`mail` أعلاه) يكون افتراضيًا `agent-native-<أول تسمية في اسم المضيف>`. اضبط `MCP_SERVER_NAME` في بيئة النشر لاختياره بنفسك — تستخدم صفحة الربط، والإعداد القابل للنسخ، ومنح تدفق الجهاز القيمة نفسها. التطبيق الذي يمرّر `mcpConnectServerName` إلى `createCoreRoutesPlugin` يحتفظ بذلك الاسم؛ ولا يُستخدم متغيّر البيئة إلا عند غياب الخيار.
...
<Diagram id="doc-block-1ras8t9" title={"مستويين من الكتالوج"} summary={"يحصل كل متصل على المستوى المدمج بشكل افتراضي؛ سطح الأداة الكامل ~ 105 قابل للاشتراك فقط. يعمل البحث عن الأدوات على سد الفجوة بحيث لا يتم إخفاء أي شيء على الإطلاق."}>
...
هذا مثال عام للسياسة. يضبط Analytics الخيار `authenticatedReads: "off"` عمدًا، ولا يعرض سوى إجراءات `connectorCatalog` التي خضعت للمراجعة.
...
يستبدل التطبيق الذي يوفّر `resolveExecutionContext` تسلسل الهوية الافتراضي هذا صراحةً. ويجب على أداة الحل الخاصة به أن ترفض بشكل مستقل المتصلين غير المتحقق منهم والضيوف والأعضاء الخارجيين ومساحات العمل غير المرتبطة قبل إرجاع هوية رئيسية.

تطابق رسائل Slack الخاصة التي تم التحقق منها عضوًا موجودًا في مؤسسة Agent Native، وتحفظ رابط هوية مساحة العمل/المستخدم قبل تشغيل الوكيل؛ ثم يحمّل سياق المستخدم/المؤسسة الناتج موارد ذلك المستخدم وتعليماته ومهاراته. يُرفض افتراضيًا عضو مساحة العمل الذي نجح جلب بياناته لكن بريده الإلكتروني مفقود أو لم ينضم بعد إلى المؤسسة. يمكن للتطبيقات ضبط `allowAnonymousOrgScopedSlackDm: true` صراحةً لتشغيل هؤلاء الأعضاء بهوية خدمة مجهولة ضمن نطاق المؤسسة — وهي طبقة الرؤية نفسها على مستوى المؤسسة التي تستخدمها القنوات المشتركة، من دون أي وصول خاص بالمستخدم — مع ملاحظة يراها الوكيل وتنبيه لمرة واحدة في Slack يشرح كيفية الحصول على وصول شخصي. لا يسمح هذا الاشتراك أبدًا بحالات فشل جلب البيانات أو المتصلين غير المتحقق منهم أو الضيوف أو الأعضاء الخارجيين (Slack Connect) أو مساحات العمل غير المرتبطة بمؤسسة؛ بل تتلقى هذه الحالات ردًا مهذبًا بالرفض بدل إسقاط الرسالة بصمت. تستخدم قنوات Slack المشتركة هوية خدمة ولا تستعير الأذونات الخاصة لأي مشارك. يطلب OAuth المُدار والنموذج المُنشأ لتطبيق Slack نطاق البوت `users:read.email`. يجب إعادة ربط/تثبيت عمليات التثبيت الحالية لمنح نطاق أُضيف حديثًا، بينما يجب أن تضيف عمليات التثبيت القديمة التي تستخدم رمز بوت النطاق يدويًا في Slack. ومن دونه، تُرفض الرسائل الخاصة ما لم يفعّل التطبيق `allowAnonymousOrgScopedSlackDm` صراحةً.
...
<Endpoint id="doc-block-1bwrrkb" method="GET" path="/_agent-native/open" summary={"المسار المفتوح للارتباط العميق — يركز المتصفح UI على سجل"} auth={"Browser session via `getSession`. The auth guard bypasses this exact path; if unauthenticated it serves login HTML at the same URL, and the form reload re-enters authenticated (no `?next=` plumbing)."} params={[
  {
    "name": "app",
    "in": "query",
    "type": "string",
...
يعمل الإدخال على تشغيل `npx @agent-native/core@latest mcp serve --app <id>`، وهو افتراضيًا ** وكيل stdio رفيع ** إلى `/mcp` للتطبيق المحلي قيد التشغيل - لذا يظل سجل الإجراء المباشر، HMR، والروابط العميقة الصحيحة المصدر الوحيد للحقيقة. قم بتمرير `--standalone` لإنشاء التسجيل قيد التشغيل بدلاً من ذلك. عندما يكتشف `npx @agent-native/core@latest mcp install` أصلًا مستضافًا (مضيف غير محلي `APP_URL` / `BETTER_AUTH_URL` / `AGENT_NATIVE_MCP_URL` في مساحة العمل `.env`)، فإنه يكتب إدخال عميل `http` يشير إلى `<origin>/mcp` باستخدام `Bearer` JWT بدلاً من إدخال stdio.
...
- تقبل نقطة النهاية `/mcp` الحالية هذا الرمز المميز مثل أي حامل آخر (راجع [MCP Protocol](/docs/mcp-protocol)) - لا توجد نقطة نهاية جديدة ولا نقل جديد.
