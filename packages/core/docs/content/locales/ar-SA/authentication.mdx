...
مدة تخزين هذا الغلاف مؤقتًا هي إعداد على مستوى عملية النشر بأكملها — انظر [SSR Caching](/docs/deployment#ssr-caching) لـ `AGENT_NATIVE_SSR_CACHE`. تقصير التخزين المؤقت أو تعطيله يغيّر المدة فقط؛ لا يزال SSR يعرض الفرع المجهول مع إزالة ملفات تعريف الارتباط.
