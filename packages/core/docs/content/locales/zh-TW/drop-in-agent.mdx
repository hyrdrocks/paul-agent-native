...
您不需要從頭開始建置原生代理。代理聊天、代理資源分頁、CLI 終端、語音輸入和所有相關基礎設施都作為少數 React 元件提供給您放入任何應用程式中。
...
將工作交給代理的按鈕（而不是執行行內 `llm()` 呼叫 - [ladder](/docs/what-is-agent-native#the-ladder) 的反模式）：
...
- **`type`** — `"content"`（預設）將工作保留在使用應用程式工具的嵌入式代理中。`"code"` 會路由到獨立的程式碼能力 frame 以變更程式碼庫原始檔；只有當主機有意授予 repository/workspace 工具時才提供寫入權限。參見 [Frames](/docs/frames)。
