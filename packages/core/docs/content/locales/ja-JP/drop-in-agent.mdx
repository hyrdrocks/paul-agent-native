...
- **`type`** — `"content"` (デフォルト) は、アプリツールを使う埋め込みエージェントで作業を保持します。`"code"` はリポジトリのソース変更用の別のコード対応フレームにルーティングします。書き込みは、ホストが repository/workspace ツールを意図的に付与した場合に限り利用できます。[Frames](/docs/frames) を参照してください。
