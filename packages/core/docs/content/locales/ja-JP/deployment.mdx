...
### Keep-warm とゼロスケール データベース {#netlify-keep-warm}

Netlify のビルドは、**毎分** `/_agent-native/health` をリクエストするスケジュール関数を出力します。これによりサーバー関数とそのデータベースがウォームに保たれ、アイドル期間後の次の訪問者がコールドスタート（Neon では 1〜5 秒）を負担しなくなります。プロビジョニング済みのコンピュートではこれが正しいデフォルトです。

**従量課金のゼロスケール データベースでは、これは誤ったデフォルトです。** 毎分の起動は自動サスペンドが一度も発動しないことを意味し、誰もアプリを使っていなくても常時稼働しているかのように課金・クォータ制限されます。Neon の無料プランではコンピュート クォータを使い切り、その後データベースはセッション中の**すべての**読み取りを拒否します。症状は「遅いアプリ」ではなく「死んだアプリ」です。

| 変数                                        | 効果                                                                                                                                                                                                                     |
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `AGENT_NATIVE_DISABLE_KEEP_WARM`            | スケジュール関数を完全にスキップします。データベースは自動サスペンドでき、アイドル期間後の次の訪問者がそのコールドスタートを負担します。                                                                                 |
| `AGENT_NATIVE_KEEP_WARM_SCHEDULE`           | 5 フィールドの cron 式（例: `*/5 * * * *`）でケイデンスを上書きします。解析できない値は、毎分へ黙って戻す代わりに**ビルドを失敗させます**。                                                                              |
| `AGENT_NATIVE_DISABLE_KEEP_WARM_BACKGROUND` | バックグラウンド関数のウォームアップのみを無効化し、サーバーはウォームに保ちます。`server` のウォームアップはリクエスト 1 回ですが、`-background` のウォームアップは新しいコンテナを起動してスキーマを再プローブします。 |

無料プランやクォータ制限のあるデータベース プランを使っている場合は、`AGENT_NATIVE_DISABLE_KEEP_WARM=1` を設定するか、より遅いケイデンスを選んでください。

### オンデマンド スキーマ DDL {#skip-ensure-tables}

各ストアは最初のデータベース接触時に `ensureTable()` を呼び出し、`information_schema` / `pg_indexes` をプローブして、本当に欠けているものだけに `CREATE TABLE` / `ADD COLUMN` / `CREATE INDEX` を発行します。長時間稼働する Node サーバーではこのコストは起動ごとに一度だけで、目に見えません。サーバーレスでは「最初の接触」が **すべてのコールドスタート** で発生し、約 57 のストアにまたがります。つまりコールドなリクエストは、実際の処理を始める前に数百回の直列ラウンドトリップを支払うことがあります。

これを減らす方法は 2 つあります。

- プローブは、テーブル・カラム・インデックスごとに 1 クエリではなく、**データベースごとに 1 回のバッチ化されたイントロスペクション**（2 クエリ）から回答されます。これは自動で、設定は不要です。
- `AGENT_NATIVE_SKIP_ENSURE_TABLES=1` はプローブと DDL の仕組みを完全にスキップします。利用できるコールドスタート遅延の改善として最大のものです。

データベース スキーマがすでにマイグレーション済みで、アプリケーションの起動時にテーブルを作成・変更したくない場合は `AGENT_NATIVE_SKIP_ENSURE_TABLES=1` を設定してください。実際のマイグレーション手順を持つ本番デプロイでは通常のケースです。

<Callout tone="risk">

**このフラグはフェイルクローズしません。また、できません。** スキーマが実際に欠けている場合、最初の実クエリはその場で修復されるのではなく、大きな音を立てて失敗します。それが狙いですが、マイグレーションの責任はあなたが負うことを意味します。テーブルの用意をアプリケーション起動に依存しているデプロイでは設定しないでください。
...
SSR を有効にした Cloudflare Worker には、Nitro のネイティブなモジュール
出力を使用します。`npx @agent-native/core@latest build` を実行し、その後
`npx wrangler deploy --config .output/server/wrangler.json` でデプロイします。
...
## SSR キャッシング {#ssr-caching}

すべての SSR HTML レスポンスとすべての React Router `.data` レスポンスは、単一の非個人化された公開シェルです。`createH3SSRHandler` はレンダリング前に Cookie を取り除くため、同じバイト列がすべての訪問者に対して正しく、パーソナライズはすべて読み込み後にクライアント側で行われます。これにより、これらのレスポンスにデフォルトで単一のハードキャッシュ ポリシーを適用しても安全になります。

```txt
cache-control: public, max-age=600, stale-while-revalidate=604800, stale-if-error=3600
```

同じ値が `cdn-cache-control` と `netlify-cdn-cache-control` にもミラーされます。これにより、リクエストごとのレンダリングの代わりに、1 つの共有 CDN エントリがサイト全体を配信するようになります。これは初回レスポンスのレイテンシに対する最大のレバーです — 以下のケースのいずれかがデプロイメントに該当しない限り、有効のままにしてください。

### `AGENT_NATIVE_SSR_CACHE` でオーバーライドする {#ssr-cache-env}

デプロイ環境で `AGENT_NATIVE_SSR_CACHE` を設定すると、デプロイメント全体のポリシーを変更できます。

| 値                                                  | 結果                                                                                     |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| 未設定、`on`、`default`、`true`、`1`                | 上記のデフォルト ポリシーのまま変更なし。推奨 — 最高のパフォーマンス。                   |
| `off`、`false`、`0`、`none`、`no-store`、`disabled` | `cache-control`、`cdn-cache-control`、`netlify-cdn-cache-control` に `no-store` を設定。 |
| 期間: `30`、`30s`、`5m`、`2h`                       | `public, max-age=<n>, stale-while-revalidate=<n>, stale-if-error=3600`。                 |

裸の数値は秒として扱われます。`stale-while-revalidate` は、カスタム期間に対して意図的に `max-age` をミラーします。短い鮮度ウィンドウにデフォルトの 7 日間の stale ウィンドウを組み合わせると、まさにオプトアウトしたはずの古さが返ってきてしまうためです。認識できない値はワーニングをログに出力し、デフォルトにフォールバックするため、タイプミスによって CDN が黙って無効化されることはありません。

以下のいずれかに該当する場合は、キャッシュを弱めるかオフにしてください。

1. **ホストがデプロイ時に CDN をパージしない場合。** Netlify と Vercel はパージしますが、一部のセルフマネージド構成ではパージされません。パージがない場合、公開済みのビルドは `max-age` に `stale-while-revalidate` を加えた期間、以前のシェルを配信し続ける可能性があります。
2. **ローダーが可変の公開データを返す場合。** ミューテーションの成功後、`useRevalidator()` またはアクション後のリダイレクトによる再フェッチが、最新のローダー出力ではなく、ブラウザにキャッシュされた `.data` のコピーを読み込んでしまうことがあります。

この設定は、公開シェルのすべてのサーフェスに適用されます: React Router の SSR HTML と `.data`、ログイン HTML シェル、`/_agent-native/speculation-rules.json` ルート、ドキュメント サイト、および forms テンプレートの公開フォーム SSR です。生成された Cloudflare Worker はこれをビルド時に解決するため、ビルドを実行する前にデプロイ環境で設定してください。

留意すべき点が 3 つあります。

- **キャッシュをオフにしても SSR がパーソナライズされるわけではありません。** Cookie は引き続きレンダリング前に取り除かれ、SSR ローダーは引き続き匿名ブランチを参照します。ユーザーごとのデータは引き続きクライアント側で解決する必要があります。この変数が制御するのはキャッシュの*期間*のみです。
- **これは設計上、ルートごとではなくデプロイメント全体の設定です。** ルートごとまたはリクエストごとのキャッシュ オーバーライドは、ある訪問者のペイロードが別の訪問者の共有 CDN エントリに紛れ込む原因になるため、フレームワークはそれを提供しません — `guard:ssr-cache-shell` がこれを強制します。
- **ミューテーション直後の最新データが必要な場合は、キャッシュを無効化するのではなく Actions を優先してください。** アプリ データは [Actions](/docs/actions) に属し、クライアントからは `useActionQuery` / `useActionMutation` で読み取り、`useDbSync()` のポーリングによって最新の状態が保たれます。これらはいずれも SSR シェル キャッシュを経由しません。SSR ローダーには引き続き公開シェルをレンダリングさせ、この変数は本当にローダー データをより新鮮にしたい場合にのみ使用してください。
...
| `AGENT_NATIVE_SSR_CACHE`                    | デプロイメント全体の SSR シェル キャッシュ ポリシー: 未設定/`on` はデフォルトのハードキャッシュを維持し、`off` は `no-store` を送信し、`30s` / `5m` のような期間を指定すると鮮度期間を短縮します。 [SSR Caching](#ssr-caching) を参照してください。                                                                                                                                                           |
| `AGENT_NATIVE_DISABLE_KEEP_WARM`            | ゼロスケール データベースが自動サスペンドできるように、Netlify の keep-warm スケジュール関数をスキップします。 [Keep-warm](#netlify-keep-warm) を参照してください。                                                                                                                                                                                                                                           |
| `AGENT_NATIVE_KEEP_WARM_SCHEDULE`           | keep-warm のケイデンスを 5 フィールドの cron 式で上書きします（デフォルト `* * * * *`）。解析できない値はビルドを失敗させます。 [Keep-warm](#netlify-keep-warm) を参照してください。                                                                                                                                                                                                                          |
| `AGENT_NATIVE_DISABLE_KEEP_WARM_BACKGROUND` | バックグラウンド関数のウォームアップのみを無効化し、サーバーはウォームに保ちます。 [Keep-warm](#netlify-keep-warm) を参照してください。                                                                                                                                                                                                                                                                       |
| `AGENT_NATIVE_DISABLE_INPROCESS_SWEEPS`     | プロセス内のバックストップ スイープ タイマー（オートメーションの再ディスパッチ、agent-teams の整合、サンドボックス実行の復旧）を停止します。サーバーレスではウォーム コンテナごとに実行されるため、クエリ レートはインスタンス数に比例します。同じ復旧を担う永続スケジューラーがすでにある場合に**のみ**設定してください。そうでないとキュー内の作業が再試行されません。                                      |
| `AGENT_NATIVE_SKIP_ENSURE_TABLES`           | オンデマンドのスキーマ DDL を完全にスキップします。すべてのコールドスタートが処理前に支払うスキーマ プローブのラウンドトリップを取り除きます。スキーマがマイグレーション済みであることが前提です — [オンデマンド スキーマ DDL](#skip-ensure-tables) を参照してください。                                                                                                                                      |
...
### 本番環境に関わる設定 {#env-required-prod}

これらは本番環境に関わる設定です。フレームワーク認証を使うスタンドアロンアプリには安定した `BETTER_AUTH_SECRET` と永続データベースが必要です。未設定の場合、公開 auth URL はリクエスト、テンプレート、ホスティングのコンテキストから推測されます。Workspace と A2A の設定は使用する機能に応じて必要になります。値がない場合はフェールクローズされるか、大きな警告とともに弱い動作に戻ります。

| 変数                                        | 説明                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `BETTER_AUTH_SECRET`                        | 32 文字以上のランダム文字列。セッション Cookie を署名し、`OAUTH_STATE_SECRET` とアプリローカルの暗号化値のフォールバックになります。既存の共有 Vault ciphertext を読むためのレガシー候補でもあります。本番環境では必須です。                                                                                |
| `BETTER_AUTH_URL`                           | このアプリの公開元の任意の上書き (例: `https://mail.example.com`)。未設定の場合はリクエスト、テンプレート、ホスティングのコンテキストから推測されます。                                                                                                                                                     |
...
| `A2A_SECRET`                                | アプリ間 A2A JSON-RPC の共有 HMAC。専用のワークスペースキーが未設定なら、共有 Vault はこの値から用途別に導出したキーを使うため、この値のローテーションは Vault キーも変更します。                                                                                                                           |
| `WORKSPACE_SECRETS_ENCRYPTION_KEY`          | 共有ワークスペース Vault に推奨される安定した AES-256-GCM キー。読むすべてのアプリに同じ値を設定し、アプリ固有の OAuth 暗号化とは分離します。                                                                                                                                                               |
| `WORKSPACE_SECRETS_ENCRYPTION_KEY_PREVIOUS` | ローテーション中に使う任意の前のワークスペースキー。新しいキーと同時に設定し、読み取りによる移行後に削除します。                                                                                                                                                                                            |
| `SECRETS_ENCRYPTION_KEY`                    | アプリ固有の暗号化と Vault 用の従来の統合キー。引き続きサポートされますが、新しいマルチアプリ環境では専用キーを使用してください。                                                                                                                                                                           |
...
「32 文字以上のランダム」とマークされたシークレット (`BETTER_AUTH_SECRET`、`OAUTH_STATE_SECRET`、`A2A_SECRET`、`WORKSPACE_SECRETS_ENCRYPTION_KEY`、`SECRETS_ENCRYPTION_KEY`、`AGENT_NATIVE_BUILDER_RELAY_SECRET`) については、次のコマンドを使用して新しい値を生成します。
...
- `database: "read"` — デフォルト。`db-schema` と `db-query` のみを登録します。
  エージェントは SQL でデータを検査しますが、書き込みには型付きアプリ actions を使用します。
- `database: "write"` または `true` — 意図的な raw SQL メンテナンス用に
  `db-exec` と `db-patch` も登録します。書き込みは現在のユーザー/組織に
  スコープされ、スキーマ変更はブロックされます。
- `database: "off"` または `false` — 生のデータベース ツールを
  エージェント サーフェスのため、アプリの actions が唯一のデータ アクセス パスになります。
- `extensions: true` — フレームワークの拡張管理 actions と
  ガイダンス（`create-extension`、`update-extension` など）をアプリで
  有効にします。認証済み REST 作成も許可するには、両方の plugins に
  設定します。拡張機能は既定で無効です。

残りのグループは真偽値で、すべて既定で有効です: `sharing`, `review`, `history`, `featureFlags`, `localization`, `audit`, `contextXray`, `userProfile`, `automation`, `docs`, `resources`, `web`, `workspaceApps`, `chat`, `email`。
音声中心のアプリや単一目的のアプリでは `frameworkTools: "minimal"` を使って
すべてを一度に無効化できます。1 つだけ残したい場合は
`frameworkTools: { preset: "minimal", resources: true }` のように書きます。
明示的なグループキーは常にプリセットより優先されます。

グループを無効にすると、対話チャット・MCP・A2A・バックグラウンド実行という
**エージェント**側のサーフェスからのみ削除されます。対応する HTTP action
ルートはマウントされたままです。UI はクライアント hooks 経由でそれらを呼ぶため、
`sharing` を無効にしても画面に出ている共有ダイアログが 404 になってはいけません。
このパリティの代償を検討してください。UI にできることはエージェントにもできる、
というのがフレームワークの契約です。共有ボタンが見えている状態での
`sharing: false` は意図的な逸脱であり、そのサーフェス自体をアプリが持たない
場合にのみ妥当です。

このオプションとは独立に、フレームワークツールは既定で最初のモデルリクエストへ
読み込まれなくなりました。検索可能なレジストリには残り、`tool-search` 経由で
読み込まれます。アプリが `initialToolNames` に明記した場合は即時に読み込まれます。

<Callout tone="warning">
  トップレベルの `databaseTools` と `extensionTools` は非推奨で、
  `frameworkTools.database` と `frameworkTools.extensions` に置き換わりました。
  引き続き解釈されますが、トップレベルのフラグと `frameworkTools` 側の同等の
  設定が矛盾する値の場合、誰も選んでいないツールサーフェスで起動するのではなく
  plugin の起動時にエラーになります。
</Callout>
...
通常のデプロイ済みアプリのエージェントは、ソースへの暗黙のアクセスを受けず、actions、データベース、MCP、設定済み統合などのアプリツールを通じて動作します。ソース変更はリポジトリの開発ワークフローで行います。埋め込み、ローカル、Builder の frame は、workspace と書き込みツールが意図的に与えられた場合にリポジトリを編集できます。
...
[Builder.io](https://www.builder.io?utm_source=agent-native&utm_medium=docs&utm_campaign=framework&utm_content=deployment) は独立したマネージドなコード対応ワークフローで、エージェントが本番環境のアプリ UI を変更できます。リポジトリを Builder.io（無料プランあり）に接続して UI の変更を直接依頼でき、再デプロイは不要です。

**仕組み:**

1. エージェントネイティブ リポジトリを Builder.io（無料プランあり）に接続します
