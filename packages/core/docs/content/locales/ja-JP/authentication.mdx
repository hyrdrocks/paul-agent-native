...
そのシェルをどれだけキャッシュするかはデプロイメント全体の設定です — `AGENT_NATIVE_SSR_CACHE` については [SSR Caching](/docs/deployment#ssr-caching) を参照してください。キャッシュを短縮または無効化しても変わるのは期間だけであり、SSR は引き続き Cookie を取り除いた匿名ブランチをレンダリングします。
