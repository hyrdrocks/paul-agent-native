...
### アプリ内の Access タブ {#access-tab}

`AgentTabsPage` をマウントするアプリでは、同じ接続フローを **Access** タブ
から利用できます。MCP URL、利用可能な場合は A2A エージェント カード、Claude、
ChatGPT、Cursor、Claude Code、Codex、その他のクライアント向けの共通設定ガイドを
表示します。アプリ内の開始点として使い、完全なフローとトークンのフォールバック
には `/mcp/connect` を開きます。これは接続設定 UI であり、一般的なアクセス管理
コンソールではありません。grants、スコープ編集、revoke はこのタブではまだ管理
できません。
...
そのブロックのサーバー ID (上記の `mail`) は、既定で `agent-native-<ホスト名の最初のラベル>` になります。自分で指定するには、デプロイの環境に `MCP_SERVER_NAME` を設定します。接続ページ、コピー可能な設定、デバイス フローの付与はすべて同じ値を使用します。`createCoreRoutesPlugin` に `mcpConnectServerName` を渡すアプリはその名前を保持します。環境変数はオプションが指定されていない場合にのみ使用されます。
